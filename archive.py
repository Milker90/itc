...
class Item:
    def __init__(self, iTMSTransporter, bundle_short_version_string, bundle_version, project_path, scheme, configuration, provisioning_profile_name, vendor_id):
        ...
        self.ipa_path = out_data_path + '/' + scheme + '_' + nowDate + '.ipa'
...
def generateItem():
    ...
    # 生成打包item
    item = Item(bundle_short_version_string, bundle_version, project_path, scheme, configuration, provisioning_profile_name, vendor_id)
...
def archive(item):
    ...
    exportIPACmd = r'xcodebuild -exportArchive -archivePath %s -exportPath %s -exportFormat ipa -exportProvisioningProfile %s' % (
